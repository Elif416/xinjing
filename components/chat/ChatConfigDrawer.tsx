...
// ChatConfigDrawer：聊天页侧边配置面板
// 通过 Props 控制开关与保存，便于实时更新 System Prompt
export function ChatConfigDrawer({
  ...
  useEffect(() => {
    setLocalName(name);
    setLocalKeywords(keywordsInput);
    setLocalTraits(emotionalTraits);
    setLocalMemories(memories);
  }, [name, keywordsInput, emotionalTraits, memories]);
            ...
            <div className="rounded-3xl border border-white/40 bg-white/60 p-4">
              ...
              <div className="mt-4 flex flex-col gap-4">
                <PersonalitySlider
                  ...
                  onChange={(value) => setLocalTraits({ ...localTraits, emotion: value })}
                  tone="light"
                />
                <PersonalitySlider
                  label="逻辑性"
                  value={localTraits.logic}
                  onChange={(value) => setLocalTraits({ ...localTraits, logic: value })}
                  tone="light"
                />
                <PersonalitySlider
                  label="幽默感"
                  value={localTraits.humor}
                  onChange={(value) => setLocalTraits({ ...localTraits, humor: value })}
                  tone="light"
                />
              </div>
            </div>
